...
import pt.uevora.clienteadministracao.models.PaginacaoDeLista;

import java.util.List;

public class ComponentesTerminal {

    private final ESTerminal esTerminal;

    public ComponentesTerminal(ESTerminal esTerminal) {
    ...
    public <T> T escolhaUm(String mensagem, List<T> opcoes, int espacos) {
        ...
        do {
            ...
            for(int i=0; i < opcoes.size(); ++i) {
                this.esTerminal.escreverMensagem(String.format("%d. %s", i+1, opcoes.get(i)), 4);
            }
            if(opcoes.isEmpty()) {
    ...
    public <T> void mostrarEEsperar(T dados, int espacos) {
        this.esTerminal.escreverMensagem(dados.toString(), espacos);
        this.esTerminal.enterParaContinuar(espacos);
    }
